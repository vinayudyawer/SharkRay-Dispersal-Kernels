...
displot<-function(data, cn, var="dis", xlab="Maximum dispersal distance (km)", ylab="Relative frequency", 
  ...
  if(nrow(dat)>3){
    ...
    if(dist=="gamma"){
      ...
      out<-data.frame(fit="gamma", mean = mean(dat[dat[,var]>0,var]), sd=sd(dat[dat[,var]>0,var]), shape=mean(a)^2/var(a), scale=var(a)/mean(a), quant=stats::quantile(dat[dat[,var]>0,var], probs=quants))
    }
    if(dist=="kernel"){
      # kernel distribution
