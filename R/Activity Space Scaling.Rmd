...
setwd("/Users/cmull/Dropbox/Work/Postdoc SFU/Data and Analysis/R")
#load data
...
```{r}
AS.mean.model <- glm(log10(mcp.km)~log10(body.mass) + lifestyle + Trophic.group + subclass, data = act.sp.mean)
summary(AS.mean.model)
...
## Testing analzying with the brms package

```{r}
library(brms)

#To examine the full Activity Space dataset we will need to perform a repeated measures pGLMM model. 

#Match up the mean activity space dataset and the phylogeny
dat.phy <- treeplyr::make.treedata(tree, act.sp.mean, name_column = "G.species")
...
inv.phylo <- MCMCglmm::inverseA(mean.tree, nodes = "TIPS", scale = TRUE)
A <- solve(inv.phylo$Ainv)
rownames(A) <- rownames(inv.phylo$Ainv)

A <- ape::vcv.phylo(mean.tree)

#run a simple model looking at body mass over scaling
actsp_trophic <- brm(
  log10(body.mass) ~ log10(mcp.km) + Trophic.group + (1|species.id), data = mean.dat, 
  family = gaussian(), cov_ranef = list(phylo = A), iter = 100000,
  prior = c(
    prior(normal(0, 10), "b"),
