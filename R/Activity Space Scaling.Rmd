...
#Model A: a simple model looking at activity space over body mass
actsp_bbk50_A <- brm(
  log10(bbk50.km) ~ log10(body.mass.kg) + (1|G.species), data = act.sp, 
  family = gaussian(), cov_ranef = list(phylo = A), iter = 10000,
  control = list(adapt_delta = 0.99),#the adapt_delta is set close to 1 (bound from 0.8-1) to reduce the divergences
  prior = priors.AS,
  save_all_pars = TRUE)
actsp_bbk50_A <- add_ic(actsp_bbk50_A, ic=c("waic", "R2", "marglik")) #add IC to model fit for comparison
saveRDS(actsp_bbk50_A, file="~/Dropbox/Work/Postdoc SFU/Data and Analysis/R/R/Model Outputs/actsp_bbk50_A_190713.rds")
actsp_bbk50_A <- readRDS("Model Outputs/actsp_bbk50_A_190713.rds")

#Model B1: activity space over body mass with lifestyle as a cactor
actsp_bbk50_B1 <- brm(
  log10(bbk50.km) ~ log10(body.mass.kg) + lifestyle + (1|G.species), data = act.sp, 
  family = gaussian(), cov_ranef = list(phylo = A), iter = 10000,
  control = list(adapt_delta = 0.99),#the adapt_delta is set close to 1 (bound from 0.8-1) to reduce the divergences
  prior = priors.AS,
  save_all_pars = TRUE)
actsp_bbk50_B1 <- add_ic(actsp_bbk50_B1, ic=c("waic", "R2", "marglik")) #add IC to model fit for comparison
saveRDS(actsp_bbk50_B1, file="~/Dropbox/Work/Postdoc SFU/Data and Analysis/R/R/Model Outputs/actsp_bbk50_B1_190713.rds")
...
saveRDS(actsp_bbk50_B2, file="~/Dropbox/Work/Postdoc SFU/Data and Analysis/R/R/Model Outputs/actsp_bbk50_B2_190713.rds")
actsp_bbk50_B2 <- readRDS("~/Dropbox/Work/Postdoc SFU/Data and Analysis/R/R/Model Outputs/actsp_bbk50_B2_190713.rds")

#Model C1: activity space over body mass with habitat as a cofactor
actsp_bbk50_C1 <- brm(
  log10(bbk50.km) ~ log10(body.mass.kg) + habitat2 + (1|G.species), data = act.sp, 
  family = gaussian(), cov_ranef = list(phylo = A), iter = 10000,
  control = list(adapt_delta = 0.99),#the adapt_delta is set close to 1 (bound from 0.8-1) to reduce the divergences
  prior = priors.AS,
  save_all_pars = TRUE)
actsp_bbk50_C1 <- add_ic(actsp_bbk50_C1, ic=c("waic", "R2", "marglik")) #add IC to model fit for comparison
saveRDS(actsp_bbk50_C1, file="~/Dropbox/Work/Postdoc SFU/Data and Analysis/R/R/Model Outputs/actsp_bbk50_C1_190713.rds")
...
saveRDS(actsp_bbk50_C2, file="~/Dropbox/Work/Postdoc SFU/Data and Analysis/R/R/Model Outputs/actsp_bbk50_C2_190713.rds")
actsp_bbk50_C2 <- readRDS("~/Dropbox/Work/Postdoc SFU/Data and Analysis/R/R/Model Outputs/actsp_bbk50_C2_190713.rds")

#Model D1: Body mass with Trophic Guild as a cofactor
actsp_bbk50_D1 <- brm(
  log10(bbk50.km) ~ log10(body.mass.kg) + Trophic.group + (1|G.species), data = act.sp, 
  family = gaussian(), cov_ranef = list(phylo = A), iter = 10000,
  control = list(adapt_delta = 0.99),#the adapt_delta is set close to 1 (bound from 0.8-1) to reduce the divergences
  prior = priors.AS,
  save_all_pars = TRUE)
actsp_bbk50_D1 <- add_ic(actsp_bbk50_D1, ic=c("waic", "R2", "marglik")) #add IC to model fit for comparison
saveRDS(actsp_bbk50_D1, file="~/Dropbox/Work/Postdoc SFU/Data and Analysis/R/R/Model Outputs/actsp_bbk50_D1_190713.rds")
...
saveRDS(actsp_bbk50_D2, file="~/Dropbox/Work/Postdoc SFU/Data and Analysis/R/R/Model Outputs/actsp_bbk50_D2_190713.rds")
