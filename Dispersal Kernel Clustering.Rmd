...
for(i in 1:nrow(MaxDisp)){
 if(i %in% 1){res<-data.frame(common_name=MaxDisp$common_name[i],
                              type=MaxDisp$type[i],
                              mean=MaxDisp$mean[i],
                              xx=dgamma(log10(seq(0,5,length=100)), scale=MaxDisp$scale[i], shape=MaxDisp$shape[i]))}
  else{
...
pp$cluster<-cutree(ppHclust, k=4)

##### dispKern from stats
dfun<-function(scale, shape, col=2, xx=NULL, add=FALSE, xlab="Maximum dispersal distance (km)", ylab="Relative frequency",...){
  if(is.null(xx)){xx<-log(seq(0,20000,length=100000)+1)}
  yy<-dgamma(xx,shape=shape,scale=scale)
...
# dat<-filter(pp, cluster %in% 4)
# for(p in 1:nrow(dat)){
#   if(p%in%1){dfun(dat$scale[p], dat$shape[p], col=adjustcolor("#FC4E07", alpha.f=0.3), ylim=c(0,2))}
#   else{dfun(dat$scale[p], dat$shape[p], col=adjustcolor("#FC4E07", alpha.f=0.3), add=T)}
# }
