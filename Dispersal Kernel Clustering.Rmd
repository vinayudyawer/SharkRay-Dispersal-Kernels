...
## Input data
fish <- read.csv("Data/Fisheries data/2018-03-16_Fisheries.csv")
imos <- read.csv("Data/Acoustic data/IMOS/2018-03-16_IMOS_PassiveTelemetry.csv")
# otn <- readRDS("Data/Acoustic data/OTN/otn_aat_detections.rds")
sat <- read.csv("Data/Satellite data/Dispersal Summaries/DispersalSummary_SatTags.csv")
```
...
satt<-as_tibble(sat) %>% 
  ...
  summarize(common_name = first(Common.Name),
            dis = max(Consecutive.Dispersal, na.rm=T))
